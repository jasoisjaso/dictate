...
class DictationTrayApp(QObject):
    ...
    def _build_menu(self):
        ...
        # collected once _build_menu returns — its menu entry (Quit!)
        # silently disappears from the tray.
        act_copy = QAction(f"{self.tr('copy_last')}  ({_pretty_key(self.copy_name)})", menu)
        act_copy.triggered.connect(self._copy_last)
        menu.addAction(act_copy)
        act_settings = QAction(self.tr("settings"), menu)
        act_settings.triggered.connect(self._open_settings)
        menu.addAction(act_settings)
        act_history = QAction(self.tr("history"), menu)
        act_history.triggered.connect(self._open_history)
        menu.addAction(act_history)
        act_guide = QAction(self.tr("guide"), menu)
        act_guide.triggered.connect(self._open_guide)
        menu.addAction(act_guide)
        menu.addSeparator()
        act_quit = QAction(self.tr("quit"), menu)
